<?php 
class Maruthi_Flickr extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array("classname"=>'widget_flickr', 'description'=>esc_html__('A widget that show last flickr photo streams', 'designthemes-core'));
		parent::__construct(false,MARUTHI_THEME_NAME.esc_html__(' Flickr Widget','designthemes-core'),$widget_options);
	}
	...
	function form($instance) {
        ...
        <p><label for="<?php echo ($this->get_field_id('flickr_id')); ?>"><?php esc_html_e('Flickr ID:','designthemes-core');?>
        ...
        <p><label for="<?php echo ($this->get_field_id('count')); ?>"><?php esc_html_e('How many entries do you want to show:','designthemes-core');?>
         ...
         <p><label for="<?php echo ($this->get_field_id('show'));?>"><?php esc_html_e('What pictures to display','designthemes-core'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('show')); ?>" name="<?php echo ($this->get_field_name('show')); ?>">
            <?php  $a = array("latest"=>esc_html__("Latest",'designthemes-core'),"random"=>esc_html__("Random",'designthemes-core'));
         ...
         <p><label for="<?php echo ($this->get_field_id('size'));?>"><?php esc_html_e('What pictures to display','designthemes-core'); ?>
         	<select class="widefat" id="<?php echo ($this->get_field_id('size')); ?>" name="<?php echo ($this->get_field_name('size')); ?>">
            <?php  $a = array("s"=>esc_html__("square",'designthemes-core'),"t"=>esc_html__("thumbnail",'designthemes-core'), "m" => esc_html__("medium",'designthemes-core'));
			foreach($a as $key => $value ):

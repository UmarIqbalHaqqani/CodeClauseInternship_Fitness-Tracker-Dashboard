...
<div v-if="paymentAvailable">
	<div class="calc-item">
			...
			<div class="calc-radio-wrapper default">
				<template v-for="payment in getStaticPayments">
					<label style="margin-right: 15px" v-if="isPaymentEnabled(payment.slug)">
						...
						<div class="calc-payment-body" v-show="payment.has_body && getMethod === payment.slug">
							...
							<div v-show="getMethod === 'twoCheckout'" style="background: transparent;" class="calc-two-checkout-wrapper">
								...
								<div class="calc-payment-details" :class="{'show-card': !twoCheckout.hideCard && twoCheckout.cardNumber.length === 19}">
		...
		<div class="calc-form-wrapper" v-if="getMethod" :style="{'margin-top': paymentDetailsList.includes(getMethod) ? '25px' : ''}">
			<div class="ccb-pro-feature-header" v-if="$store.getters.getUnusedFields.length === 0 && getMethod && ['paypal', 'stripe', 'cash_payment'].includes(getMethod)" style="margin-bottom: 10px"><?php esc_html_e( 'Order Details', 'cost-calculator-builder-pro' ); ?></div>
			<div class="calc-default-form">
				<template v-if="$store.getters.getUnusedFields.length === 0 && getMethod && paymentDetailsList.includes(getMethod)">
	...
	<div class="ccb-btn-wrap" style="margin-top: 20px; position: relative">
		...
		<div class="ccb-btn-container calc-buttons <?php echo ! empty( $general_settings['invoice'] ) && $general_settings['invoice']['emailButton'] && ! $general_settings['invoice']['showAfterPayment'] && $general_settings['invoice']['use_in_all'] ? esc_attr( 'pdf-enable' ) : ''; ?>" v-else>

...
class CCBPayments {
	...
	private static function validate() {
		if ( is_string( $_POST['data'] ) ) {
			self::$params = json_decode( stripslashes( $_POST['data'] ), true );
		}
	...
	public static function makePaid( $orderId, $paymentData, $d_totals = array(), $o_totals = array() ) {
		...
		update_option( 'calc_meta_data_order_' . $orderId, $meta_data );
	...
	protected static function getPaymentSettings() {
		$general_settings = self::getPaymentGeneralSettingsBySlug( self::$paymentMethod );
		$settings         = self::getPaymentSettingsBySlug( self::$paymentMethod );

		if ( ! empty( $general_settings ) && ! empty( $general_settings['use_in_all'] ) ) {

...
class ClassShortcode extends BaseController {
	...
	public function init(  ) {
		add_shortcode( 'gbclass', [ $this, 'gbclass_shortcode_render' ] );
	}

	public function gbclass_shortcode_render( $atts ) {
		...
		$this->scId          = $scID;
		$scClassSettingsMeta = get_post_meta( $scID );
		$rand                = absint( wp_rand() );
		$layoutID            = 'gbclass-shortcode-container-' . $rand;
		$metas = Helper::gbClassMetaScBuilder( $scClassSettingsMeta );
		$layout = $metas['layout'];
		$exclude_layouts = GymBuilderClass::exclude_global_query_layout();
		if ( ! in_array( $layout,$exclude_layouts)){
			...
			$containerClass  = 'gbclass-shortcode-container gym-builder-grid';
			$rowClass  = 'gbclass-shortcode-wrapper columns-' .esc_attr($metas['grid_columns']);
			$html = '<div class="' . esc_attr( $containerClass." ".esc_attr($metas['layout']) ) . '" id="' . esc_attr( $layoutID ) . '" ' . $containerAttr . '>';

			$query = ( new ShortcodeQuery() )->buildArgs( $scID, $metas, self::$class_post_type,self::$class_taxonomy )->get_gb_shortcode_posts();
			...
			if ($query->have_posts()){
				...
				while ( $query->have_posts() ) {
					$query->the_post();
					$metas['class_id'] = get_the_ID();
					$html .= Functions::render('shortcode/class/layouts/'. $layout,$metas,true);
				}
				ob_start();
				$html .= ob_get_contents();
				ob_end_clean();
			...
			return GymBuilderClass::get_schedule_routine($this->scId,$metas,$layoutID);
		}
